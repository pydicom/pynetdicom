...
class TestElementPath:
    ...
    def test_pathing(self):
        ...
        paths = ["(0000,0000)=", "0000,0000=", "CommandGroupLength="]
        for path in paths:
        ...
        paths = ["(0000,0000)=120", "0000,0000=120", "CommandGroupLength=120"]
        for path in paths:
            elem = ElementPath(path)
            assert elem.tag == Tag(0x0000, 0x0000)
            assert elem.keyword == "CommandGroupLength"
        ...
        paths = ["(300a,00b0)[0]=", "300a,00b0[0]=", "BeamSequence[0]="]
        for path in paths:
            elem = ElementPath(path)
    ...
    def test_update_existing_non_sequential(self):
        ...
        ref.BeamSequence[0].DACSequence[0].BeamNumber = 1
        ref.BeamSequence[0].DACSequence[2].BeamNumber = 3
        ref.BeamSequence[1].DACSequence = [Dataset(), Dataset(), Dataset(), Dataset()]
