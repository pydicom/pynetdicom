...
- C-FIND: ``AE.on_c_find(dataset, context, assoc_info)`` and ``AE.on_c_find_cancel()``
- C-GET: ``AE.on_c_get(dataset, context, assoc_info)`` and ``AE.on_c_get_cancel()``
- C-MOVE: ``AE.on_c_move(dataset, move_aet, context, assoc_info)`` and ``AE.on_c_move_cancel()``

Where *dataset* is a pydicom Dataset object, *context* is the Presentation
Context used to transfer *dataset* and *assoc_info* is a dict containing
information about the association (such as the peer's IP address and AE title).
