...
* Fixed the :class:`~pynetdicom.transport.AssociationServer` not properly
  closing its sockets (:gh:`pynetdicom/issues/382`)
* Improved the robustness of determining the local IP address (:gh:`pynetdicom/issues/394`)
* Fixed not being able to set an *Implementation Version Name* of length 1
  (:gh:`pynetdicom/issues/414`)
...
* Stopped the QR Move SCP from associating with the move destination when no
  sub-operations were required (:gh:`pynetdicom/issues/436`)
* Fixed a possible race condition during DIMSE message exchange (:gh:`pynetdicom/issues/435`)
* Fixed the association acceptor selecting the first matching proposed
  ...
  during presentation context negotiation (:gh:`pynetdicom/issues/445`)
* Fixed incorrect Pending status value for Modality Worklist (:gh:`pynetdicom/issues/462`)
* Fixed an ``InvalidEventError`` due to two A-ABORT messages being sent. Issue
  ...
  both expire while sending a DIMSE request. (:gh:`pynetdicom/issues/460`)
* Fixed datasets encoding using *Deflated Explicit VR Little Endian* not
  transferring correctly (:gh:`pynetdicom/issues/482`)
...
* Add new SOP classes up to 2019e version of the standard (:gh:`pynetdicom/issues/424`)
...
* If an A-ABORT is received while waiting for a DIMSE message the
  ...
  waiting for the DIMSE timeout. (:gh:`pynetdicom/issues/430`)
