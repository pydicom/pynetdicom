...
* Overhaul of ``echoscp.py``, ``echoscu.py``, ``storescp.py`` and
  ``storescu.py``
* Added non-toy implementations for ``findscu.py``, ``getscu.py`` and
  ``movescu.py`` apps
...
* Added a customisable :class:`~pynetdicom.status.Status` convenience class
  for comparing, returning and yielding status values (:issue:`454`)
