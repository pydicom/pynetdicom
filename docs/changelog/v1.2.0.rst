...
* Consolidated and improved the robustness of the transport related code
  ...
  * It should now be possible to both request a C-MOVE operation and act as the
    ...
    (:issue:`5`, :issue:`181`)
* Added ``AE.active_requestors`` and ``AE.active_acceptors`` properties which
  return a list of active requestor/acceptor Association threads.
...
* ``AE.active_associations`` changed from an attribute to a property.
